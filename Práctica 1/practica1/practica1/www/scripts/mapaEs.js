...
var settingsMapa = {
    "async": true,
    "crossDomain": true,
    "url": "https://opendata.aemet.es/opendata/api/valores/climatologicos/inventarioestaciones/todasestaciones?api_key=" + keyEstaciones,
    "method": "GET",
    "headers": {
...
function InicializarMapa() {
    ...
    $.ajax(settingsMapa).done(function (response) {
        ...
        $.ajax(response.datos).done(function (response) {
            ...
            var i = 1;
            datosMapa = JSON.parse(response);
            var centro = map.getCenter();

            // Al principio la informacion no se tiene que ver, se debe esperar a pulsar el pushpin
